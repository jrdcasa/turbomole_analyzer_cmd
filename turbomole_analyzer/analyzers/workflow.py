...
class WorkflowAnalyzer:
    ...
    def analyze_job(self, job_dir: Path) -> JobResults:
        ...
        # Parse theory and control metadata
        control_data = ControlParser().parse_metadata(job_dir / "control")
        if job_type == "single_point" and control_data["task_type"] != "single_point":
            job_type = control_data["task_type"]
        ...
        e_total_zpe = (e_elec + zpe) if (e_elec and zpe) else None
        ...
        # Dynamic metadata attachment
        res.__dict__["functional"] = control_data["functional"]
        res.__dict__["dispersion"] = control_data["dispersion"]
        res.__dict__["basis_set"] = control_data["basis_set"]
    ...
    def process_project(self, project_dir_name: str):
        ...
        for job_dir in sorted(project_path.glob("job_*")):
            if job_dir.is_dir():
                ...
                if e_elec is None and job_id == "job_0001" and "job_0000" in self.jobs:
                    e_elec = self.jobs["job_0000"].electronic_energy

                zpe = None
                if job_type == "frequency":
                    zpe = FrequencyParser().parse(job_dir / "aoforce.out")

                nmr_raw = NMRParser().parse(job_dir / "mpshift.out")
                nmr_data = NMRData(method=job_type, chemical_shifts=nmr_raw) if nmr_raw else None

                e_total_zpe = (e_elec + zpe) if (e_elec and zpe) else None

                self.jobs[job_id] = JobResults(
                    job_id=job_id,
                    job_type=job_type,
                    electronic_energy=e_elec,
                    zpe_correction=zpe,
    ...
    def print_individual_reports(self):
        ...
        for job_id, job in self.jobs.items():
            ...
            print(f"  - Electronic Energy (E_elec) : {job.electronic_energy if job.electronic_energy else 'N/A'} Ha")
    ...
    def print_relative_energy_report(self, reference_job_id: str):
        ...
        ref_total_with_zpe = (ref_e_elec + ref_zpe) if (ref_e_elec and ref_zpe) else None
        hartree_to_kcal = 627.509

        print(f"Reference Electronic Energy (E_el): {ref_e_elec} Ha")
        print(f"Reference ZPE Correction Found    : {ref_zpe if ref_zpe else 'None'} Ha")
        print(f"Reference Total (E_el + ZPE)      : {ref_total_with_zpe if ref_total_with_zpe else 'N/A'} Ha")
        print("-" * 80)
        ...
        for job_id, job in self.jobs.items():
            ...
            abs_energy_str = f"{job.electronic_energy:16.6f}" if job.electronic_energy else "N/A"

            # 2. Delta Elec + ZPE calculation
            d_zpe_str = "N/A"
            current_zpe = job.zpe_correction if job.zpe_correction is not None else ref_zpe

            if job.electronic_energy and current_zpe and ref_total_with_zpe:
